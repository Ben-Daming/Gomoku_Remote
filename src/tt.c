#include "../include/tt.h"
#include <stdlib.h>
#include <string.h>
#include <stdio.h>

static TTEntry* tt_table = NULL;
static uint64_t tt_size = 0; // Number of entries
static uint64_t tt_mask = 0; // Mask for fast indexing (size - 1)

// Helper to pack/unpack move
static inline uint8_t packMove(Position p) {
    if (p.row == -1 || p.col == -1) return 0xFF;
    return (uint8_t)((p.row << 4) | p.col);
}

static inline Position unpackMove(uint8_t packed) {
    if (packed == 0xFF) return (Position){-1, -1};
    return (Position){(packed >> 4) & 0xF, packed & 0xF};
}

void tt_init(int size_mb) {
    if (tt_table) free(tt_table);
    
    // Calculate number of entries
    // Ensure size is power of 2 for fast indexing
    uint64_t bytes = (uint64_t)size_mb * 1024 * 1024;
    uint64_t count = bytes / sizeof(TTEntry);
    
    // Find nearest power of 2 <= count
    tt_size = 1;
    while (tt_size * 2 <= count) {
        tt_size *= 2;
    }
    tt_mask = tt_size - 1;
    
    tt_table = (TTEntry*)calloc(tt_size, sizeof(TTEntry));
    printf("TT Initialized: %d MB, %lu entries\n", size_mb, tt_size);
}

void tt_free() {
    if (tt_table) {
        free(tt_table);
        tt_table = NULL;
    }
}

void tt_clear() {
    if (tt_table) {
        memset(tt_table, 0, tt_size * sizeof(TTEntry));
    }
}

int tt_probe(uint64_t key, int rem_depth, int alpha, int beta, int* out_val, Position* out_move) {
    if (!tt_table) return 0;

    uint64_t index = key & tt_mask;
    TTEntry* entry = &tt_table[index];

    if (entry->key == key) {
        // Always retrieve best move for ordering
        *out_move = unpackMove((uint8_t)entry->best_move);

        // Check if entry is usable for cutoff
        if (entry->rem_depth >= rem_depth) {
            int score = entry->value;
            
            // Adjust mate scores if necessary (omitted for now, assuming raw scores)

            if (entry->flag == TT_FLAG_EXACT) {
                *out_val = score;
                return 1;
            }
            if (entry->flag == TT_FLAG_LOWERBOUND && score >= beta) {
                *out_val = score;
                return 1;
            }
            if (entry->flag == TT_FLAG_UPPERBOUND && score <= alpha) {
                *out_val = score;
                return 1;
            }
        }
    } else {
        *out_move = (Position){-1, -1};
    }
    return 0;
}

void tt_save(uint64_t key, int rem_depth, int value, int flag, Position best_move) {
    if (!tt_table) return;

    uint64_t index = key & tt_mask;
    TTEntry* entry = &tt_table[index];

    // Replacement Strategy:
    // 1. Always replace if empty (key == 0)
    // 2. Replace if new depth is greater or equal
    // 3. (Optional) Replace if same depth (to keep fresh)
    // Note: We are not using 'age' yet, but structure supports it.
    
    if (entry->key == 0 || rem_depth >= entry->rem_depth) {
        // Write key last? For single thread it doesn't matter.
        // For simple thread safety without locks, we can write key last.
        
        entry->rem_depth = rem_depth;
        entry->value = value;
        entry->flag = flag;
        entry->best_move = packMove(best_move);
        // entry->age = ...; 
        
        // Write key last
        entry->key = key;
    }
}

void tt_prefetch(uint64_t key) {
    if (!tt_table) return;
    uint64_t index = key & tt_mask;
    __builtin_prefetch(&tt_table[index]);
}
